...
def begin():
	...
	if(('org.apache.flink.runtime.taskmanager.TaskManager' not in list_process_names) and (host_name == 'zoo1')):
		print ("Flink Task Manager is not yet started! Retrying 'jcmd' command")
		list_process_ids = []
		list_process_names = []
		time.sleep(.5)
		begin()
		...
		if host_name=="zoo1":
			subprocess.Popen(['./stream-bench.sh', 'FLINK_TEST'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
