...
def profile():
	...
	for i in range(0, len(list_process_ids)):
		file_name = 'filename=/home/ubuntu/eranga/developments/Dev11-storm-1.x.x/jfr/' + file_name_const + "-test1-" + str(i + 1) + '-' + list_process_names[i] + '.jfr'
        	call(['jcmd', list_process_ids[i], 'VM.unlock_commercial_features'])
        	call(['jcmd', list_process_ids[i], 'JFR.start duration=150s', file_name])
        	print (file_name)
		time.sleep(.5)
